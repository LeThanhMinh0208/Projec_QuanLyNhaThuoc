...
public class KiemKeChonLoExcelExporter {

    public static class LoRow {
        public final String tenThuoc;
        public final String maLoThuoc;
        public final int soLuongTon;

        public LoRow(String tenThuoc, String maLoThuoc, int soLuongTon) {
            this.tenThuoc   = tenThuoc   != null ? tenThuoc   : "";
            this.maLoThuoc  = maLoThuoc  != null ? maLoThuoc  : "";
    ...
    public static void xuatFileMau(List<LoRow> dsLo, File file) throws Exception {
        try (OutputStream os = Files.newOutputStream(file.toPath());
            ...
            write(zos, "xl/worksheets/sheet1.xml",      sheet(dsLo));
        }
    }
    ...
    private static String sheet(List<LoRow> dsLo) {
        ...
        sb.append("<cols>");
        sb.append(colDef(1, 1, 42)); // A: Tên Thuốc
        sb.append(colDef(2, 2, 22)); // B: Số Lô
        sb.append(colDef(3, 3, 14)); // C: SL Tồn
        sb.append(colDef(4, 4, 10)); // D: Chọn (yellow input)
        sb.append("</cols>\n");
        sb.append("<sheetData>\n");
        ...
        // Row 1: Title
        merges.add("A1:D1");
        sb.append(row(r++, new CV[]{ new CV("DANH SÁCH LÔ THUỐC KIỂM KÊ - NHÀ THUỐC LONG NGUYÊN", 0) }));

        // Row 2: Instructions
        merges.add("A2:D2");
        sb.append(row(r++, new CV[]{ new CV(
            "Đánh dấu 'x' vào cột CHỌN để chọn lô kiểm kê. Để trống nghĩa là không chọn.", 1) }));
        ...
        if (dsLo != null) {
            for (LoRow lo : dsLo) {
                sb.append(row(r++, new CV[]{
                    ...
                    new CV(lo.maLoThuoc,                   5),
                    new CV(String.valueOf(lo.soLuongTon),  6),
                    new CV("",                             7) // yellow: user fills x / 1
                }));
            }
        }
    ...
    private static String workbook() {
        return "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
            ...
            "<sheets><sheet name=\"ChonLoKiemKe\" sheetId=\"1\" r:id=\"rId1\"/></sheets>" +
            "</workbook>";
    }
