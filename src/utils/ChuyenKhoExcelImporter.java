...
public class ChuyenKhoExcelImporter {
    ...
    private static void parseSheet(InputStream is, Map<Integer, String> ss, KetQua result) throws Exception {
        ...
        for (int i = 0; i < rowNodes.getLength(); i++) {
            ...
            if (rowNum == 3) { result.nguoiVanChuyen = vals[1].trim(); continue; }
            if (rowNum == 4) { result.khoXuat        = vals[1].trim(); continue; }
            if (rowNum == 5) { result.khoNhan        = vals[1].trim(); continue; }
            ...
            // Dòng dữ liệu: A=tenThuoc, B=soLo
