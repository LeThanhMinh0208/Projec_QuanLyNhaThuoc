...
public class TraHangNCCExcelImporter {
    ...
    public static class DongDuLieu {
        ...
        public String soLo     = "";
        public int soLuongTra  = -1; // -1 = ô trống → dùng toàn bộ tồn kho
    }
    ...
    private static void parseSheet(InputStream is, Map<Integer, String> ss, KetQua result) throws Exception {
        ...
        for (int i = 0; i < rowNodes.getLength(); i++) {
            ...
            // Đọc 6 cột A-F (index 0-5)
            String[] vals = {"", "", "", "", "", ""};
            NodeList cNodes = rowEl.getElementsByTagName("c");
            for (int j = 0; j < cNodes.getLength(); j++) {
                Element c = (Element) cNodes.item(j);
                int colIdx = getColIdx(c.getAttribute("r"));
                if (colIdx >= 6) continue;
                vals[colIdx] = getCellValue(c, ss);
            }

            // Row 3: Nhà cung cấp ở col B (index 1)
            if (rowNum == 3) { result.tenNCC = vals[1].trim(); continue; }
            // Row 4: Lý do ở col B (index 1)
            if (rowNum == 4) { result.lyDo   = vals[1].trim(); continue; }
            ...
            // Dòng dữ liệu: A=tenThuoc, B=soLo, D=soLuongTra
            ...
            // Cột F (index 5): SL Trả — -1 nếu để trống
            String slStr = vals[5].replaceAll("[^\\d]", "").trim();
            if (!slStr.isEmpty()) {
                try { dong.soLuongTra = Integer.parseInt(slStr); } catch (NumberFormatException ignored) {}
            }
