...
public class DonDatHangExcelImporter {
    ...
    private static void parseSheet(InputStream is, Map<Integer, String> ss, KetQua result) throws Exception {
        ...
        for (int i = 0; i < rowNodes.getLength(); i++) {
            ...
            // Row 3: NCC in column B
            if (rowNum == 3) {
                result.nhaCungCap = vals[1].trim();
                continue;
            }
            // Row 4: Ghi Chú in column B
            if (rowNum == 4) {
                result.ghiChu = vals[1].trim();
                continue;
            }

            // Find header row: column A contains "Tên Thuốc"
            if (!headerFound) {
            ...
            // Data rows: A=TenThuoc, B=DonVi (ignored), C=SoLuong
            String tenThuoc = vals[0].trim();
            if (tenThuoc.isEmpty()) continue;

            int sl = 0;
            try {
                String slStr = vals[2].replaceAll("[^\\d]", "").trim();
                if (!slStr.isEmpty()) sl = Integer.parseInt(slStr);
