...
public class GUI_DanhMucKhoController implements Initializable {
    ...
    @FXML void moTrangNhapKho(ActionEvent event) {
        ...
        SceneUtils.switchPage("/gui/main/GUI_NhapKho.fxml");
    }
    @FXML void handleChuyenTrangXuatKho(ActionEvent event) {
        if (!utils.UserSession.getInstance().hasPermission("QLK.XUAT_KHO")) {
            AlertUtils.showAlert(Alert.AlertType.WARNING, "Không có quyền", "Bạn không có quyền truy cập trang Xuất Kho.");
            return;
        }
        SceneUtils.switchPage("/gui/main/GUI_XuatKho.fxml");
    }
