...
public class GUI_QuanLyLoThuocController implements Initializable {
    ...
    // 🚨 KHAI BÁO COMBOBOX LỌC NHÀ CUNG CẤP 🚨
    ...
    @FXML private Label lblNgayNhap, lblGiaNhap, lblNhaCungCap;
    @FXML private Label lblSlNhapBanDau, lblSlDaBan, lblSlTraNcc, lblSlXuatHuy, lblSlDoiTra;

    // 🚨 2 BIẾN CHO KIỂM KÊ 🚨
    @FXML private Label lblKkDu, lblKkThieu;
        ...
        try {
            ...
            if (dsNcc != null) {
                for (NhaCungCap ncc : dsNcc) {
                    if (ncc.getTenNhaCungCap() != null) tenNccList.add(ncc.getTenNhaCungCap());
                }
            }
    ...
    private void setupSmartFilter() {
        ...
        txtTonTu.textProperty().addListener((obs, old, newV) -> { 
            if (!newV.matches("\\d*")) txtTonTu.setText(newV.replaceAll("[^\\d]", ""));
            updateFilter(); 
        });
        txtTonDen.textProperty().addListener((obs, old, newV) -> { 
            if (!newV.matches("\\d*")) txtTonDen.setText(newV.replaceAll("[^\\d]", ""));
            updateFilter(); 
        });
    ...
    private void updateFilter() {
        filteredData.setPredicate(lo -> {
            ...
            boolean matchStatus = "Tất cả".equals(status) || 
                                 ("Đang hoạt động".equals(status) && lo.getTrangThai() == 1) ||
                                 ("Đã khóa".equals(status) && lo.getTrangThai() == 0);

            String kho = cbKho.getValue();
            boolean matchKho = "Tất cả".equals(kho) || 
                              ("Kho Bán Hàng".equals(kho) && "KHO_BAN_HANG".equals(lo.getViTriKho())) ||
                              ("Kho Dự Trữ".equals(kho) && "KHO_DU_TRU".equals(lo.getViTriKho()));
    ...
    @FXML void toggleFilter(ActionEvent event) {
        boolean isVisible = paneFilter.isVisible();
        paneFilter.setVisible(!isVisible);
        paneFilter.setManaged(!isVisible);
    }

    @FXML void clearFilter(ActionEvent event) {
        dpHsdTu.setValue(null); dpHsdDen.setValue(null);
        txtTonTu.clear(); txtTonDen.clear();
        cbKho.getSelectionModel().selectFirst();
    ...
    private void setupTable() {
        ...
        colTenThuoc.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getThuoc() != null ? cellData.getValue().getThuoc().getTenThuoc() : "Không xác định"));
    ...
    private void setupColorsAndImages() {
        colAnh.setCellFactory(param -> new TableCell<LoThuoc, LoThuoc>() {
            ...
            @Override protected void updateItem(LoThuoc lo, boolean empty) {
                ...
                if (empty || file == null || file.trim().isEmpty()) {
                    iv.setImage(null); setGraphic(empty ? null : box);
                } else {
                    try {
        ...
        colHanSuDung.setCellFactory(column -> new TableCell<LoThuoc, Date>() {
            @Override protected void updateItem(Date item, boolean empty) {
                ...
                if (empty || item == null) { setText(null); setStyle(""); }
                    ...
                    // NẾU ĐÃ HẾT HẠN -> Đen xì, gạch ngang
                    if (daysBetween < 0) setStyle("-fx-text-fill: #7f8c8d; -fx-font-weight: bold; -fx-alignment: CENTER; -fx-strikethrough: true;"); 
                    // CẬN DATE (<= 90 ngày) -> Màu Đỏ chót
                    else if (daysBetween <= 90) setStyle("-fx-text-fill: #e74c3c; -fx-font-weight: bold; -fx-alignment: CENTER;");
                    else setStyle("-fx-text-fill: #2c3e50; -fx-font-weight: normal; -fx-alignment: CENTER;");
                }
            }
        });
        ...
        colSoLuong.setCellFactory(column -> new TableCell<LoThuoc, Integer>() {
            @Override protected void updateItem(Integer item, boolean empty) {
                ...
                if (empty || item == null) { setText(null); setStyle(""); }
                    ...
                    if (item == 0) setStyle("-fx-text-fill: #7f8c8d; -fx-font-weight: bold; -fx-alignment: CENTER; -fx-strikethrough: true;");
                    else if (item < 100) setStyle("-fx-text-fill: #e74c3c; -fx-font-weight: bold; -fx-alignment: CENTER;");
                    else setStyle("-fx-text-fill: #2c3e50; -fx-font-weight: normal; -fx-alignment: CENTER;");
                }
            }
        });
            ...
            @Override protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) { setText(null); setStyle(""); }
                    ...
                    if ("KHO_BAN_HANG".equals(item)) {
                        setText("Kho Bán Hàng"); setStyle("-fx-text-fill: #27ae60; -fx-font-weight: bold; -fx-alignment: CENTER;");
                    } else {
                        setText("Kho Dự Trữ"); setStyle("-fx-text-fill: #e67e22; -fx-font-weight: bold; -fx-alignment: CENTER;");
                    }
                }
            }
        });
        ...
        colThaoTac.setCellFactory(param -> new TableCell<LoThuoc, LoThuoc>() {
            ...
            @Override protected void updateItem(LoThuoc lo, boolean empty) {
                super.updateItem(lo, empty);
                if (empty || lo == null) setGraphic(null);
                else {
    ...
    private void setupRowSelection() {
        tableLoThuoc.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                paneChiTiet.setVisible(true); paneChiTiet.setManaged(true);
                lblGiaNhap.setText(String.format("%,.0f VNĐ", newSelection.getGiaNhap()));
                String donViGoc = (newSelection.getThuoc() != null) ? newSelection.getThuoc().getDonViCoBan() : "";
                loadThongKeTheLo(newSelection.getMaLoThuoc(), donViGoc);
            } else {
                paneChiTiet.setVisible(false); paneChiTiet.setManaged(false);
            }
        });
    }

    private void loadThongKeTheLo(String maLoThuoc, String donVi) {
        String sql = "SELECT l.ngayNhapKho, ncc.tenNhaCungCap AS tenNCC, l.soLuongTon, " +
            ...
            "FROM LoThuoc l LEFT JOIN NhaCungCap ncc ON l.maNhaCungCap = ncc.maNhaCungCap WHERE l.maLoThuoc = ?";
        ...
        try (Connection con = ConnectDB.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                lblNhaCungCap.setText(rs.getString("tenNCC") != null ? rs.getString("tenNCC") : "Không xác định");
                ...
                // Logic tìm số nhập ban đầu (Duy nhất HEAD có: Tính bù trừ kiểm kê)
                if (slNhap == 0 && (tonKho > 0 || slBan > 0 || slTraNcc > 0 || slXuatHuy > 0)) {
                    int netChenhLech = kkDu - kkThieu;
    ...
    private void kiemTraVaKhoaLoTuDong(List<LoThuoc> list) {
        for (LoThuoc lo : list) {
            if (lo.getTrangThai() == 1) {
                boolean canKhoa = lo.getSoLuongTon() <= 0;
                if (!canKhoa && lo.getHanSuDung() != null) {
                    canKhoa = java.time.temporal.ChronoUnit.DAYS.between(LocalDate.now(), lo.getHanSuDung().toLocalDate()) < 0;
                }
                if (canKhoa) {
    ...
    @FXML void handleLamMoi(ActionEvent event) {
        txtTimKiem.clear(); cbTrangThai.getSelectionModel().selectFirst();
        clearFilter(null); loadData();
    }

    private void xuLyKhoaLo(String maLo, int trangThaiMoi) {
        if (daoLoThuoc.capNhatTrangThaiLo(maLo, trangThaiMoi)) {
            AlertUtils.showAlert(Alert.AlertType.INFORMATION, "Thành công", trangThaiMoi == 0 ? "Đã khóa lô thành công!" : "Đã mở khóa lô thành công!");
            loadData();
        } else {
