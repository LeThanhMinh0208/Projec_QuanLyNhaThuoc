...
import utils.NhapKhoExcelImporter;
...
public class GUI_NhapKhoController {
    ...
    void handleImportCSV(ActionEvent event) {
        ...
        fileChooser.setTitle("Chọn file Excel nhập kho");
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Excel Files (*.xlsx)", "*.xlsx"));
        File file = fileChooser.showOpenDialog(txtNhaCungCap.getScene().getWindow());
        if (file == null) return;
        try {
            NhapKhoExcelImporter.KetQua kq = NhapKhoExcelImporter.docFileExcel(file.getAbsolutePath());

            // Tìm đơn đặt hàng theo mã đọc từ file
            if (kq.maDon.isEmpty()) {
                AlertUtils.showAlert(Alert.AlertType.ERROR, "Lỗi file",
                    "File không chứa mã đơn đặt hàng. Vui lòng dùng file xuất từ Chi Tiết Đơn Đặt Hàng.");
                return;
            }

            DonDatHang donTimThay = null;
            for (DonDatHang d : daoDon.getAllDonDatHang()) {
                if (d.getMaDonDatHang().equalsIgnoreCase(kq.maDon)) {
                    donTimThay = d;
                    break;
                }
            }
            if (donTimThay == null) {
                AlertUtils.showAlert(Alert.AlertType.ERROR, "Không tìm thấy đơn",
                    "Không tìm thấy đơn đặt hàng [" + kq.maDon + "] trong hệ thống.");
                return;
            }

            // Chuyển sang tab Tạo Phiếu và tải đơn
            tabTaoPhieu.setSelected(true);
            hienThiChiTietDon(donTimThay);

            // Chọn đơn trong ComboBox
            loadDonChoNhap();
            for (DonDatHang d : cbMaDon.getItems()) {
                if (d.getMaDonDatHang().equals(donTimThay.getMaDonDatHang())) {
                    cbMaDon.getSelectionModel().select(d);
                    break;
                }
            }

            // Điền thông tin từ file vào danh sách thuốc
            int countSuccess = 0;
            for (NhapKhoExcelImporter.DongDuLieu dong : kq.dsDong) {
                for (ChiTietDonDatHang ct : listChiTietHienTai) {
                    if (ct.getThuoc().getTenThuoc().equalsIgnoreCase(dong.tenThuoc)) {
                        if (dong.soLuongNhan >= 0)       ct.setSoLuongDaNhan(dong.soLuongNhan);
                        if (!dong.maLo.isEmpty())          ct.setMaLo(dong.maLo);
                        if (!dong.ngaySanXuat.isEmpty())    ct.setNgaySanXuatTemp(dong.ngaySanXuat);
                        if (!dong.hanDung.isEmpty())         ct.setHanSuDung(dong.hanDung);
                        countSuccess++;
                        break;
                    }
                }
            ...
            tinhToanTongTienHienThi();
            AlertUtils.showAlert(Alert.AlertType.INFORMATION, "Thành công",
                "Đã tải đơn [" + kq.maDon + "] và nạp thông tin cho "
                + countSuccess + "/" + listChiTietHienTai.size() + " thuốc từ file Excel.");
        } catch (Exception e) {
            e.printStackTrace();
            AlertUtils.showAlert(Alert.AlertType.ERROR, "Lỗi đọc file", "Không thể đọc file Excel: " + e.getMessage());
        }
    }
}